...
def init_hf_bert_biencoder(args, **kwargs): # Huggingface BERT 기반 Bi-Encoder 초기화
    if importlib.util.find_spec("transformers") is None:
        raise RuntimeError("Please install transformers lib")
    from .hf_models import get_bert_biencoder_components

    return get_bert_biencoder_components(args, **kwargs) # BERT Bi-Encoder 구성 요소 반환
...
BIENCODER_INITIALIZERS = { # 모델 초기화 함수 매핑, 3가지 모델 지원
